...
void Processor_ManageClockInterruption();
...
int Processor_FetchInstruction() {
	...
	if (MMU_readMemory()==MMU_SUCCESS) {
		...
		int i; int isInvalidInstruction = 1;
		for (i = 0; i < 13; i++) {
			if (codedInstruction[i] != '0' && codedInstruction[i] != '\b') {
				isInvalidInstruction = 0;
				break;
			}
		}
...
void Processor_DecodeAndExecuteInstruction() {
	...
	switch (operationCode) {
		...
		// Instruction MEMADD
		case 'm':
			// Now a badass called MMU has come to our system...,
			// he controls every memory change & we must beg him
			// for everything we need to do with it.
			MMU_SetMAR(operand2);
			MMU_readMemory();
			// Good boy! He will have dropped the information
			// requested into the MBR cell, so we can take it &
			// move it to our momma 'Accumulator'.
			Processor_SetAccumulator(registerMBR_CPU.cell);
			
			// Momma wants some more food, lets give her!
			tempAcc = Processor_GetAccumulator();
			Processor_SetAccumulator(tempAcc + operand1);
			// Sometimes momma explodes, so we are told to call 
			// emergencies. Have you exploded, momma?
			Processor_CheckOverflow(tempAcc, operand1);
			// Now we are on vacation, we command the next minion
			// to follow our job.
			registerPC_CPU++;
			break;
		...
		// Instruction HALT
		case 'h':
			// Is the protected mode activated?
		...
		case 'o':
			...
			if (Processor_PSW_BitState(EXECUTION_MODE_BIT)) {
				...
				// Show message: " (PID: executingProcessID, PC: registerPC_CPU, Accumulator: registerAccumulator_CPU, PSW: registerPSW_CPU [Processor_ShowPSW()]\n
	...
	int i;
		...
		for (i=0;i<INTERRUPTTYPES;i++)
			...
			if (Processor_GetInterruptLineStatus(i)) {
				...
				// Activate interruption masked status
...
// Put the exception interrupt line to a high level &
// put the registerB_CPU to the exception type value
